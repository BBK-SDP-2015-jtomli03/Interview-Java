 ...
 *//*  The == operator compares an objects location in memory.
    ...
    Sometimes we want .equals to compare the objects state (data) instead -> we can do this by overriding the equals
    method to compare the objects fields.
    If we override .equals we also need to override hashCode so that it generates an equal hashcode for equal objects.
    This is especially important for hash based collections (eg. HashMap, HashSet etc) which use hashcodes to store and
    retrieve data.
    When overriding hashCode you need to use the same set of fields used to compute .equals.
    See Alien class for implementation of hashCode.
